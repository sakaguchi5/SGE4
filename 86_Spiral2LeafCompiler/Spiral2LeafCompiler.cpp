...
base::Result<FrozenHierarchyLeafV1,LeafCompileErrorV1> CompileDynamicInvocationSourceV1(const hierarchy::RigidHierarchySemanticV1& h,const verification::VerifiedHierarchyRepresentationV1& verified)
{
    ...
    const std::uint32_t bytes=h.boneCount*32u;sem::SemanticBuilder b;auto dynamic=b.AddDynamicBuffer("Spiral2.DynamicMotors",bytes,16);auto output=b.AddExternalBuffer("Spiral2.DynamicSource.Output",bytes,32);if(!dynamic||!output)return base::Result<FrozenHierarchyLeafV1,LeafCompileErrorV1>::Failure(Err("resources","dynamic source resource construction failed"));auto ud=b.AddUse(dynamic.Value(),sem::Effect::Read,sem::ViewRole::ConstantData);auto uo=b.AddUse(output.Value(),sem::Effect::Write,sem::ViewRole::StorageBuffer);sem::ProgramInterface pi;pi.parameters={{{0},"Dynamic",sem::ProgramParameterKind::ConstantBuffer,sem::ShaderStage::Compute,0,bytes,16},{{1},"Output",sem::ProgramParameterKind::UnorderedBuffer,sem::ShaderStage::Compute,0,0,1}};std::ostringstream ss;ss<<"static const uint BoneCount="<<h.boneCount<<"u;\ncbuffer Dynamic:register(b0){float4 Words["<<h.boneCount*2<<"];};\n"<<CommonTypes<<R"hlsl(
RWStructuredBuffer<Motor> Output:register(u0);[numthreads(64,1,1)]void CSMain(uint3 id:SV_DispatchThreadID){uint i=id.x;if(i>=BoneCount)return;Motor m;m.qr=Words[i*2];m.qd=Words[i*2+1];Output[i]=m;})hlsl";
    auto program=b.AddComputeProgram("Spiral2.DynamicInvocationSource.V1",std::move(pi),{ss.str(),{}, {},"CSMain"});if(!program)return base::Result<FrozenHierarchyLeafV1,LeafCompileErrorV1>::Failure(Err("program",program.Error()));const std::array ops={sem::WorkOperand{sem::WorkOperandKind::ProgramParameter,ud.Value(),{0}},sem::WorkOperand{sem::WorkOperandKind::ProgramParameter,uo.Value(),{1}}};auto work=b.AddComputeWorkGeneric("Spiral2.DynamicInvocationSource.Work.V1",program.Value(),ops,(h.boneCount+63)/64,1,1);if(!work)return base::Result<FrozenHierarchyLeafV1,LeafCompileErrorV1>::Failure(Err("work",work.Error()));return Finish("L0.DynamicInvocationSource",std::move(b),bytes,32,32,verified.CertificateIdentity());
}
...
base::Result<FrozenHierarchyLeafGroupV1,LeafCompileErrorV1> CompileVerifiedHierarchyLeafGroupV1(const hierarchy::RigidHierarchySemanticV1& h,const verification::VerifiedHierarchyRepresentationV1& verified)
{
    ...
    const std::string matrixHierarchy=R"hlsl(StructuredBuffer<Matrix34> Input:register(t0);RWStructuredBuffer<Matrix34> Output:register(u0);[numthreads(64,1,1)]void CSMain(uint3 id:SV_DispatchThreadID){if(id.x!=0)return;for(uint j=0;j<BoneCount;++j){uint i=Order[j];if(Parent[i]<0)Output[i]=Input[i];else Output[i]=MMul(Output[Parent[i]],Input[i]);}})hlsl";
    ...
    const std::string motorHierarchy=R"hlsl(StructuredBuffer<Motor> Input:register(t0);RWStructuredBuffer<Motor> Output:register(u0);[numthreads(64,1,1)]void CSMain(uint3 id:SV_DispatchThreadID){if(id.x!=0)return;for(uint j=0;j<BoneCount;++j){uint i=Order[j];if(Parent[i]<0)Output[i]=Input[i];else Output[i]=DqMul(Output[Parent[i]],Input[i]);}})hlsl";
