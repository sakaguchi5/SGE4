...
bool WriteText(const std::filesystem::path& path, std::string_view text)
{
    ...
    return static_cast<bool>(sge4_5::base::WriteAllBytes(
...
int BuildEvidence(std::string& manifest)
{
    ...
    sge4_5::d3d12::D3D12Backend removalBackend({true, false});
    ...
    auto removed = runtime::RecoverStaticComposition(
        removalLoaded.Value(), sge4_5::runtime::DeviceRecoveryMode::ForceRemovalForTest);
    if (!removed) return 8;
    auto retry = runtime::RecoverStaticComposition(
        removalLoaded.Value(), sge4_5::runtime::DeviceRecoveryMode::RetryAdapterReacquisition);
    if (!retry) return 9;

    std::ostringstream out;
    out << "identity=SGE4-Level4v1-Canonical-R5-Final-Qualification-v1\n";
    out << "frozen-composition-sha256=" << sge4_5::base::ToHex(sge4_5::base::Sha256(artifact.Value())) << '\n';
    out << "pre-recovery-output-sha256=" << sge4_5::base::ToHex(sge4_5::base::Sha256(read0.Value().bytes)) << '\n';
    out << "post-recovery-output-sha256=" << sge4_5::base::ToHex(sge4_5::base::Sha256(read1.Value().bytes)) << '\n';
    ...
    out << "actual-removal-awaiting=" << (removed.Value().device.stateAfter == sge4_5::runtime::DeviceRuntimeState::AwaitingAdapter ? 1 : 0) << '\n';
    out << "retry-awaiting=" << (retry.Value().device.stateAfter == sge4_5::runtime::DeviceRuntimeState::AwaitingAdapter ? 1 : 0) << '\n';
    const auto body = out.str();
    out << "evidence-aggregate=" << sge4_5::base::ToHex(sge4_5::base::Sha256(
