...
constexpr std::string_view EvidenceMagic = "SGE4-5.Spiral2.MeasurementEvidence.V1";
...
{
    CandidatePreparationV1 result;result.kind=kind;const auto generationBegin=Clock::now();auto raw=planner::PlanCandidateGraphV1(hierarchyValue,kind);const auto generationEnd=Clock::now();if(!raw)throw std::runtime_error(raw.Error());const auto verifierBegin=Clock::now();auto verified=verification::VerifyCandidateGraphV1(hierarchyValue,raw.Value());const auto verifierEnd=Clock::now();if(!verified)throw std::runtime_error(verified.Error());const auto freezeBegin=Clock::now();auto group=leaf::CompileVerifiedHierarchyLeafGroupV1(hierarchyValue,verified.Value());if(!group)throw std::runtime_error(group.Error().stage+": "+group.Error().message);for(const auto& item:group.Value().leaves){auto valid=leaf::ValidateFrozenHierarchyLeafV1(item);if(!valid)throw std::runtime_error(valid.Error().stage+": "+valid.Error().message);result.packageBytes+=item.packageBytes.size();const std::uint64_t count=item.outputStride==16?static_cast<std::uint64_t>(hierarchyValue.boneCount)*5u:hierarchyValue.boneCount;result.logicalIntermediateBytes+=count*item.outputStride;}const auto freezeEnd=Clock::now();result.generationNanoseconds=Nanoseconds(generationBegin,generationEnd);result.verifierNanoseconds=Nanoseconds(verifierBegin,verifierEnd);result.freezeNanoseconds=Nanoseconds(freezeBegin,freezeEnd);result.dynamicUploadBytes=static_cast<std::uint64_t>(hierarchyValue.boneCount)*contracts::DynamicMotorStrideBytesV1;result.dispatchCount=raw.Value().proposedDispatchCount;result.verifiedCertificate=verified.Value().CertificateIdentity();return result;
}
...
{
    const auto& contract=runtime.loaded.Artifact().ValidatedContract().Contract();const auto source=MeasurementLeaf(contract,runtime.scenario.leaves[0].role);const auto outputFlow=MeasurementFlow(contract,runtime.scenario.outputFlowKey);Require(source.IsValid()&&outputFlow.IsValid(),"candidate measurement Composition identity lookup failed");canrt::StaticCompositionFrameInvocation invocation;invocation.frameNumber=runtime.nextFrame++;invocation.dynamicData={{source,runtime.scenario.leaves[0].lockedDynamicSlot,contracts::SerializeDynamicLocalMotorPaletteV1(palette)},{source,runtime.scenario.leaves[0].lockedReferenceSlot,corpus::SerializeReferencePointsV1(std::vector<corpus::ObservedPointV1>(reference.begin(),reference.end()))}};auto submitted=canrt::SubmitStaticComposition(runtime.loaded,invocation);if(!submitted)throw std::runtime_error(submitted.Error().stage+": "+submitted.Error().message);auto readback=canrt::ReadStaticCompositionBuffer(runtime.loaded,outputFlow);if(!readback)throw std::runtime_error(readback.Error().stage+": "+readback.Error().message);auto profiles=backend.ConsumeTimestampProfileSamples();Require(profiles.size()==runtime.scenario.leaves.size(),"candidate timestamp count mismatch");const auto firstInstance=std::min_element(profiles.begin(),profiles.end(),[](const auto& a,const auto& b){return a.instanceOrdinal<b.instanceOrdinal;})->instanceOrdinal;CandidateExecution result;result.candidate.kind=runtime.scenario.kind;result.candidate.orderPosition=orderPosition;for(const auto& profile:profiles){const auto relative=profile.instanceOrdinal-firstInstance;Require(relative<runtime.scenario.leaves.size(),"candidate timestamp instance ordinal mismatch");LeafTimingSampleV1 value;value.role=runtime.scenario.leaves[static_cast<std::size_t>(relative)].role;value.kind=RoleKind(value.role);value.commandRecordingNanoseconds=profile.commandRecordingNanoseconds;value.gpuNanoseconds=profile.gpuNanoseconds;value.dispatchCount=profile.dispatchCount;value.barrierCount=profile.barrierCount;result.leaves.push_back(value);if(static_cast<std::uint32_t>(value.kind)!=0){result.candidate.commandRecordingNanoseconds+=value.commandRecordingNanoseconds;result.candidate.gpuTotalNanoseconds+=value.gpuNanoseconds;result.candidate.dispatchCount+=value.dispatchCount;result.candidate.barrierCount+=value.barrierCount;}}auto points=DecodeMeasurementPoints(readback.Value().bytes,hierarchyValue.boneCount*5u);result.candidate.numericMaxAbsoluteError=CandidateMaxError(reference,points);return result;
}
...
{
    MeasurementSampleV1 result;result.run=run;result.cycle=cycle;result.candidateOrder=OrderName(order);const auto begin=Clock::now();for(std::uint32_t position=0;position<3;++position){auto* runtime=*std::find_if(runtimes.begin(),runtimes.end(),[&](const auto* value){return value->scenario.kind==order[position];});auto executed=ExecuteCandidate(position,hierarchyValue,palette,reference,*runtime,backend);result.compositionMaterializationNanoseconds+=runtime->materializationNanoseconds;result.leaves.insert(result.leaves.end(),executed.leaves.begin(),executed.leaves.end());result.candidates.push_back(executed.candidate);}result.compositionEndToEndNanoseconds=static_cast<double>(Nanoseconds(begin,Clock::now()));return result;
}

void WritePreparation(base::BinaryWriter& w,const CandidatePreparationV1& v){w.WriteU32(static_cast<std::uint32_t>(v.kind));w.WriteU64(v.generationNanoseconds);w.WriteU64(v.verifierNanoseconds);w.WriteU64(v.freezeNanoseconds);w.WriteU64(v.packageBytes);w.WriteU64(v.dynamicUploadBytes);w.WriteU64(v.logicalIntermediateBytes);w.WriteU32(v.dispatchCount);WriteDigest(w,v.verifiedCertificate);}
base::Result<CandidatePreparationV1,std::string> ReadPreparation(base::BinaryReader& r){CandidatePreparationV1 v;auto a=r.ReadU32();auto b=r.ReadU64();auto c=r.ReadU64();auto d=r.ReadU64();auto e=r.ReadU64();auto f=r.ReadU64();auto g=r.ReadU64();auto h=r.ReadU32();auto i=ReadDigest(r);if(!a||!b||!c||!d||!e||!f||!g||!h||!i)return base::Result<CandidatePreparationV1,std::string>::Failure("preparation section truncated");v.kind=static_cast<candidate::CandidateKindV1>(a.Value());v.generationNanoseconds=b.Value();v.verifierNanoseconds=c.Value();v.freezeNanoseconds=d.Value();v.packageBytes=e.Value();v.dynamicUploadBytes=f.Value();v.logicalIntermediateBytes=g.Value();v.dispatchCount=h.Value();v.verifiedCertificate=i.Value();return base::Result<CandidatePreparationV1,std::string>::Success(v);}
...
{
    std::ofstream f(path,std::ios::binary);if(!f)throw std::runtime_error("failed to create measurement_samples.csv");f<<"case,bones,run,cycle,order,order_position,candidate,leaf,command_recording_ns,gpu_ns,composition_e2e_ns,materialization_ns,dispatches,barriers,numeric_max_abs_error\r\n"<<std::setprecision(17);for(const auto& c:e.cases)for(const auto& s:c.samples)for(const auto& l:s.leaves){if(static_cast<std::uint32_t>(l.kind)==0)continue;const auto found=std::find_if(s.candidates.begin(),s.candidates.end(),[&](const auto& v){return v.kind==l.kind;});f<<c.id<<','<<c.boneCount<<','<<s.run<<','<<s.cycle<<','<<s.candidateOrder<<','<<(found==s.candidates.end()?0:found->orderPosition)<<','<<KindName(l.kind)<<','<<l.role<<','<<l.commandRecordingNanoseconds<<','<<l.gpuNanoseconds<<','<<s.compositionEndToEndNanoseconds<<','<<s.compositionMaterializationNanoseconds<<','<<l.dispatchCount<<','<<l.barrierCount<<','<<(found==s.candidates.end()?0:found->numericMaxAbsoluteError)<<"\r\n";}
}
}
...
base::Result<MeasurementEvidenceV1,std::string> RunRealGpuMeasurementV1(const MeasurementConfigV1& config)
{
    try{
        ...
        MeasurementEvidenceV1 evidence;evidence.config=config;evidence.captureUnixNanoseconds=static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count());evidence.adapter=CaptureAdapterProfile(config.adapterIndex);evidence.observationContractIdentity=hierarchy::ObservationContractIdentityV1();d3d12::D3D12Backend backend({false,false,true});base::BinaryWriter profileWriter;WriteString(profileWriter,"SGE4-5.Spiral2.MeasurementProfile.V1");WriteDigest(profileWriter,evidence.adapter.fingerprint);WriteDigest(profileWriter,evidence.observationContractIdentity);profileWriter.WriteU32(config.warmupCyclesPerOrder);profileWriter.WriteU32(config.measurementCyclesPerOrder);profileWriter.WriteU32(config.runCount);WriteString(profileWriter,config.clockPolicy);
...
{
    MeasurementEvidenceV1 e;e.captureUnixNanoseconds=1;e.config={1,1,1,0,"self-test"};e.adapter.description="Synthetic hardware adapter";e.adapter.vendorId=1;e.adapter.timestampFrequency=1000000;e.adapter.fingerprint=base::Sha256(std::as_bytes(std::span("synthetic",9)));e.observationContractIdentity=e.adapter.fingerprint;e.measurementProfileIdentity=e.adapter.fingerprint;for(std::uint32_t ci=0;ci<6;++ci){MeasurementCaseV1 c;c.id="M"+std::to_string(ci);c.topology="synthetic";c.boneCount=8u<<std::min(ci,4u);c.hierarchyDepthCount=ci+1;c.hierarchyIdentity=e.adapter.fingerprint;c.invocationIdentity=e.adapter.fingerprint;for(const auto kind:CandidateOrders[0]){CandidatePreparationV1 p;p.kind=kind;p.packageBytes=100+static_cast<std::uint32_t>(kind);p.dynamicUploadBytes=c.boneCount*32u;p.logicalIntermediateBytes=c.boneCount*static_cast<std::uint32_t>(kind)*16u;p.dispatchCount=kind==candidate::CandidateKindV1::DirectPgaHierarchy?2:3;p.verifiedCertificate=e.adapter.fingerprint;c.preparation.push_back(p);}for(const auto& order:CandidateOrders){MeasurementSampleV1 s;s.candidateOrder=OrderName(order);s.compositionEndToEndNanoseconds=1000;for(std::uint32_t position=0;position<3;++position){CandidateTimingSampleV1 v;v.kind=order[position];v.orderPosition=position;v.gpuTotalNanoseconds=100+10*static_cast<std::uint32_t>(v.kind);v.dispatchCount=v.kind==candidate::CandidateKindV1::DirectPgaHierarchy?2:3;s.candidates.push_back(v);}c.samples.push_back(s);}e.cases.push_back(c);}auto written=WriteMeasurementArtifactsV1(e,outputDirectory);if(!written)return written;auto read=ReadMeasurementEvidenceV1(outputDirectory/"spiral2_measurement_evidence_v1.bin");if(!read)return base::Result<void,std::string>::Failure(read.Error());if(read.Value().cases.size()!=6||read.Value().cases[0].samples.size()!=6)return base::Result<void,std::string>::Failure("measurement evidence roundtrip failed");return WriteDecisionEvidenceReportV1(read.Value(),outputDirectory);
}
}
