...
int RunDiamond(Evidence& evidence)
{
    ...
    for (std::size_t index = 0; index < submitted.Value().leaves.size(); ++index)
    {
        if (submitted.Value().leaves[index].deviceEpoch != submitted.Value().deviceEpoch ||
            submitted.Value().executionOrder[index] !=
                artifact.Value().plan.schedule[index].leaf)
            return 7;
    }
    ...
    auto corrupt = artifact.Value().fileBytes;
...
int RunPresenter(Evidence& evidence)
{
    ...
    if (!inputId.IsValid() || !artifact.Value().contract.presenterLeaf.IsValid()) return 31;
    auto window = sge4::platform::Win32Window::Create(
        L"SGE4 L4V1-F9 Schema 18 Presenter", 64, 64);
    if (!window) return 32;
    ...
    auto submitted = runtime18::SubmitStaticComposition(loaded.Value(), {0, {}});
    if (!submitted || submitted.Value().leaves.size() != 2 ||
        submitted.Value().executionOrder.back() != artifact.Value().contract.presenterLeaf)
        return 34;
...
std::string EvidenceManifest(const Evidence& evidence)
{
    ...
    output << "identity=SGE4-L4V1-F7-F9-Static-DAG-Runtime-v1\n";
...
int main(int argc, char** argv)
{
    ...
    std::cout << "L4V1-F9 Schema 18 Static Package DAG Runtime tests passed\n";
