...
| S6-P02 | Dense mask, compact list and block-list masks reconstruct the same set. | Planned CU4 Candidate family |
...
| S6-P04 | Candidate output write set is exactly `S`. | CU2 WARP proof; CU3 binds the exact write-set authority to Semantic, set, artifact and Resource |
...
| S6-P08 | Cross-set, cross-role, cross-Candidate and cross-epoch seals are rejected. | CU3 PASSED for set, Target, Observation, artifact, Resource, role, layout and device epoch; cross-Candidate extends in CU4 |
