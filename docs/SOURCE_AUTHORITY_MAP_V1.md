...
| Composition contract／flow | `CompositionModel` | schedule等を提案 | DAG、single writer、presenter等を独立検証 | flat `SGE4UNI 2.0` Composition Core + `CompositionCertificate` | scheduleを再計画しない |
| Dynamic membership／delta | `DynamicModelArtifact` input/history types | exact setを導出 | exact setを独立再導出 | `FrozenDynamicInvocationPackage` | Frozen Invocationだけを消費 |
