...
| Composition contract／flow | `CompositionModel` | schedule等を提案 | DAG、single writer、presenter等を独立検証 | flat `SGE4UNI 2.6` Composition Core + `CompositionCertificate` | scheduleを再計画しない |
| Texture2D Flow shape | 埋込みSchema 17 Leaf endpointから`CompositionModel`が再導出 | allocationを提案 | kind／format／extent／single writerを独立検証 | SGE4UNI 2.6 Contract／Plan schema 2 | shared Texture、state、completion、pitch写像を機械適用 |
| Texture2D UAV route | 埋込みSchema 17 Leafのformat／UAV view／UnorderedWrite stateから`CompositionModel`が再導出 | allocation／handoffを提案 | RGBA32F、single writer、UnorderedWrite→ShaderReadを独立検証 | SGE4UNI 2.6 Contract／Plan schema 2 | shared UAV Texture、descriptor、state、completionを機械適用 |
...
| Multi-target Verified Dynamic route | `CompositionModel` Dynamic Contract schema 5 | Canonical payloadと全routeをproposalへbind | route順序／slice／target／payload set／identityを独立再導出 | `SGE4UNI 2.6` route table + `SGE4INV 1.5` Execution Payload schema 2 | verified Update／Clearを全private shadowsへ機械適用し一括Commit |
...
|---|---|---|---|---|---|
| Conditional predicate／branch membership | Composition Dynamic Contract | 変更しない | graph closureとCanonical性を検査 | SGE4UNI 2.6 schema 3 | 再解釈しない |
...
|---|---|---|---|---|
| logical Texture extent／format | Composition Contract | Leaf endpointsとの完全一致を再検査 | SGE4UNI 2.6 Contract Data schema 2 | 再解釈しない |
| shared Texture allocation | Composition Planner | Contractと一致するpacked bytes／ownershipを再検査 | SGE4UNI 2.6 Verified Decision Data schema 2 | fixed D3D12 Textureへ物質化 |
...
|---|---|---|---|---|---|
| Indirect target／maximum | Composition Dynamic Contract | 変更しない | Schema 17 Command／operationとの一致を検査 | SGE4UNI 2.6 Dynamic Contract schema 5 | 再選択しない |
