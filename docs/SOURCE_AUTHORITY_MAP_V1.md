...
| Composition contract／flow | `CompositionModel` | schedule等を提案 | DAG、single writer、presenter等を独立検証 | flat `SGE4UNI 2.5` Composition Core + `CompositionCertificate` | scheduleを再計画しない |
| Texture2D Flow shape | 埋込みSchema 17 Leaf endpointから`CompositionModel`が再導出 | allocationを提案 | kind／format／extent／single writerを独立検証 | SGE4UNI 2.5 Contract／Plan schema 2 | shared Texture、state、completion、pitch写像を機械適用 |
| Texture2D UAV route | 埋込みSchema 17 Leafのformat／UAV view／UnorderedWrite stateから`CompositionModel`が再導出 | allocation／handoffを提案 | RGBA32F、single writer、UnorderedWrite→ShaderReadを独立検証 | SGE4UNI 2.5 Contract／Plan schema 2 | shared UAV Texture、descriptor、state、completionを機械適用 |
| Dynamic membership／delta | `DynamicModelArtifact` input/history types | exact setを導出 | exact setを独立再導出 | `SGE4INV 1.4` | Frozen Invocationだけを消費 |
| Verified Dynamic payload route | `CompositionModel` Dynamic Contract | exact Update payloadをproposalへbind | payload set／identityを独立再導出 | `SGE4UNI 2.5` route + `SGE4INV 1.4` Execution Payload | verified Update／Clearをprivate shadowへ機械適用 |
...
|---|---|---|---|---|---|
| Conditional predicate／branch membership | Composition Dynamic Contract | 変更しない | graph closureとCanonical性を検査 | SGE4UNI 2.5 schema 3 | 再解釈しない |
| Region selection／enabled Leaf集合 | 直接指定不可 | exact setから導出 | exact setから独立再導出 | SGE4INV 1.4 Conditional Execution | Seal済み集合を機械適用 |
...
|---|---|---|---|---|
| logical Texture extent／format | Composition Contract | Leaf endpointsとの完全一致を再検査 | SGE4UNI 2.5 Contract Data schema 2 | 再解釈しない |
| shared Texture allocation | Composition Planner | Contractと一致するpacked bytes／ownershipを再検査 | SGE4UNI 2.5 Verified Decision Data schema 2 | fixed D3D12 Textureへ物質化 |
...
|---|---|---|---|---|---|
| Indirect target／maximum | Composition Dynamic Contract | 変更しない | Schema 17 Command／operationとの一致を検査 | SGE4UNI 2.5 schema 4 | 再選択しない |
| exact work count | exact Transition set | countとX／Y／Zを提案 | exact setから独立再導出 | SGE4INV 1.4 Indirect Dispatch | 再計算／clampしない |
