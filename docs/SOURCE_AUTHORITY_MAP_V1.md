...
| Composition contract／flow | `CompositionModel` | schedule等を提案 | DAG、single writer、presenter等を独立検証 | flat `SGE4UNI 2.4` Composition Core + `CompositionCertificate` | scheduleを再計画しない |
| Texture2D Flow shape | 埋込みSchema 17 Leaf endpointから`CompositionModel`が再導出 | allocationを提案 | kind／format／extent／single writerを独立検証 | SGE4UNI 2.4 Contract／Plan schema 2 | shared Texture、state、completion、pitch写像を機械適用 |
...
| Verified Dynamic payload route | `CompositionModel` Dynamic Contract | exact Update payloadをproposalへbind | payload set／identityを独立再導出 | `SGE4UNI 2.4` route + `SGE4INV 1.4` Execution Payload | verified Update／Clearをprivate shadowへ機械適用 |
...
|---|---|---|---|---|---|
| Conditional predicate／branch membership | Composition Dynamic Contract | 変更しない | graph closureとCanonical性を検査 | SGE4UNI 2.4 schema 3 | 再解釈しない |
...
|---|---|---|---|---|
| logical Texture extent／format | Composition Contract | Leaf endpointsとの完全一致を再検査 | SGE4UNI 2.4 Contract Data schema 2 | 再解釈しない |
| shared Texture allocation | Composition Planner | Contractと一致するpacked bytes／ownershipを再検査 | SGE4UNI 2.4 Verified Decision Data schema 2 | fixed D3D12 Textureへ物質化 |
...
|---|---|---|---|---|---|
| Indirect target／maximum | Composition Dynamic Contract | 変更しない | Schema 17 Command／operationとの一致を検査 | SGE4UNI 2.4 schema 4 | 再選択しない |
