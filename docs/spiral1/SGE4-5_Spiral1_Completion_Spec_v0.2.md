...
# Spiral 1：PGA剛体変換表現保持実験 完成仕様 v0.2

- 状態: Foundation-integrated Completion Specification
- 日付: 2026-07-19
...
### Stage S1-0：Research Contract Freeze

- 本仕様を固定
- Corpus V1を固定
- Observation Contract V1を固定
- Program template interfaceを固定
- 非目標を固定

完成条件: C++変更なしで、入力、出力、候補、Verifier、テスト、完成宣言を説明できる。

### Stage S1-1：Math and Semantic Foundation
...
完成条件: Semantic canonicalizationとreference corpusがfresh processで決定的。

### Stage S1-2：Representation Candidate Generation

- MatrixExpandedCompute candidate
- DirectPgaMotorCompute candidate
- Candidate manifest / identity

完成条件: 同じSemantic identityから異なる二候補が生成される。

### Stage S1-3：Independent Representation Authority

- Matrix independent rederivation
- Motor coefficient verification
- Program template allowlist
- seal authority
- mutation tests

完成条件: Raw Candidateは実行不能で、すべての主要改変を拒否する。

### Stage S1-4：Frozen Leaf Qualification

- L0-L3 Leaf Package生成
- Schema / Runtime compatibility
- leaf-only WARP tests

完成条件: 各Leafが単独にPackage validationと既定観測を通過。

### Stage S1-5：Level 4 v1 Composition
...
完成条件: Level 4 v1を拡張せずに4 Leafを接続できる。

### Stage S1-6：WARP Observation Corpus

- S00-S15実行
- Comparison Record readback
- deterministic CPU aggregation
- tolerance gate

完成条件: 全corpusがObservation Contractを満たす。

### Stage S1-7：Recovery and Fresh Process

- controlled recovery
- actual RemoveDevice
- stale epoch rejection
- Debug / Release manifest comparison

完成条件: Recovery後も同じFrozen Compositionから同じ観測を再現。

### Stage S1-8：Real GPU Measurement

- Adapter fingerprint profile
- ABBA benchmark
- numerical and performance report

完成条件: 性能優劣に関係なく再現可能な比較reportが生成される。

### Stage S1-9：Decision Report

- Level 4 v1で十分だったか
- Representation保持に実際の価値があったか
- 次の螺旋で何を試すか
- Level 4へ追加すべき能力があるか

完成条件: 次の実装を推測ではなく観測結果から選べる。
