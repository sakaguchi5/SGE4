...
| Integration | every `.vcxproj` appears exactly once in `SemanticGpuEngine4-5.sln`; Debug and Release main-solution builds | duplicate/missing project, missing configuration mapping, stage-specific runner reference | final runner executes R1-R5 and Stage 0D from the main solution build |
