...
namespace
{
sge4_5::package::PackageBuildInput MakeInput()
{
    using namespace sge4_5::package;
    PackageBuildInput input;
    sge4_5::base::BinaryWriter manifest;
...
int main()
{
    auto first = sge4_5::package::PackageWriter::Write(MakeInput());
    auto second = sge4_5::package::PackageWriter::Write(MakeInput());
    if (!first || !second || first.Value() != second.Value()) { std::cerr << "writer determinism failed\n"; return 1; }
    auto read = sge4_5::package::PackageReader::Read(first.Value());
    if (!read) { std::cerr << read.Error().message << '\n'; return 2; }

    sge4_5::package::PackageBuildInput roundTrip;
    ...
    {
        sge4_5::package::PackageSectionInput copy;
    ...
    auto rewritten = sge4_5::package::PackageWriter::Write(std::move(roundTrip));
    if (!rewritten || rewritten.Value() != first.Value()) { std::cerr << "round trip bytes differ\n"; return 3; }

    auto badMagic = first.Value();
    badMagic[0] ^= std::byte{1};
    if (sge4_5::package::PackageReader::Read(std::move(badMagic))) { std::cerr << "bad magic accepted\n"; return 4; }
    auto corrupt = first.Value();
    corrupt.back() ^= std::byte{1};
    if (sge4_5::package::PackageReader::Read(std::move(corrupt))) { std::cerr << "corruption accepted\n"; return 5; }

    auto unknownInput = MakeInput();
    sge4_5::package::PackageSectionInput unknown;
    unknown.kind = static_cast<sge4_5::package::SectionKind>(0x00006000u);
    unknown.flags = static_cast<std::uint32_t>(sge4_5::package::SectionFlags::Required | sge4_5::package::SectionFlags::ExecutionAffecting);
    unknown.alignment = 8;
    unknown.bytes = {std::byte{1}};
    unknownInput.sections.push_back(std::move(unknown));
    auto unknownBytes = sge4_5::package::PackageWriter::Write(std::move(unknownInput));
    if (!unknownBytes || sge4_5::package::PackageReader::Read(std::move(unknownBytes).Value())) { std::cerr << "unknown required section accepted\n"; return 6; }
