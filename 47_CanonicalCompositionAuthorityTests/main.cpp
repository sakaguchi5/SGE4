...
namespace canonical = sge4::composition::canonical;
...
std::vector<std::byte> CompileLeafPackage()
{
    ...
    auto package = sge4::compiler::CompileCanonical(graph.Value(), Profile());
    if (!package)
...
sge4::composition::FrozenCompositionBuildInput MakeLowLevelContainerInput(
    const canonical::ValidatedCompositionContract& contract,
...
void ExpectForgedArtifactRejected(
    ...
    std::string_view message)
{
    auto bytes = sge4::composition::FrozenCompositionWriter::Write(std::move(input));
...
int main(int argc, char** argv)
{
    try
    {
        ...
        ExpectResignedPlanRejected(validated, raw, [](auto& plan) {
            plan.waits[0].signal = sge4::base::InvalidIndex;
        ...
        {
            auto written = sge4::base::WriteAllBytes(argv[2], frozenBytes);
        ...
        std::cout << "Artifact SHA-256: "
                  << sge4::base::ToHex(sge4::base::Sha256(frozenBytes)) << '\n';
        return 0;
    }
