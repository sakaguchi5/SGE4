...
namespace sge4::level5::vertical2
{
namespace sem = sge4::semantic;
namespace comp = sge4::composition;
namespace dyn = sge4::dynamic;

inline constexpr std::string_view StateLeafKey = "level5/v2/state-writer";
inline constexpr std::string_view TextureLeafKey = "level5/v2/texture-writer";
inline constexpr std::string_view TemporalLeafKey = "level5/v2/temporal-producer";
inline constexpr std::string_view ObservationLeafKey = "level5/v2/observation";

inline constexpr std::string_view StateEndpoint = "level5/v2/state/output";
inline constexpr std::string_view TextureEndpoint = "level5/v2/texture/output";
inline constexpr std::string_view TemporalStateInputEndpoint = "level5/v2/temporal/state-input";
inline constexpr std::string_view TemporalCurrentEndpoint = "level5/v2/temporal/current";
inline constexpr std::string_view ObservationPreviousEndpoint = "level5/v2/observation/previous";
inline constexpr std::string_view ObservationStateEndpoint = "level5/v2/observation/state";
inline constexpr std::string_view ObservationOutputEndpoint = "level5/v2/observation/output";

inline constexpr std::string_view StateFlowKey = "level5/v2/state";
inline constexpr std::string_view TextureFlowKey = "level5/v2/texture";
inline constexpr std::string_view TemporalFlowKey = "level5/v2/temporal-history";
inline constexpr std::string_view ObservationFlowKey = "level5/v2/observation-output";
    ...
    std::uint32_t universe)
{
    ...
    auto dynamicState = builder.AddDynamicBuffer(
        "L5V2.State.Dynamic", totalBytes, MemberBytes, MemberBytes);
    auto compactIndices = builder.AddDynamicBuffer(
        "L5V2.State.CompactIndices", indexBytes, IndexBytes, IndexBytes);
    auto output = builder.AddExternalBuffer(
        "L5V2.State.Output", totalBytes, MemberBytes);
    if (!dynamicState || !compactIndices || !output)
        return base::Failure<CompiledLeaf, std::string>(
            "Level 5実験2 State Bufferが検証または実行の契約に違反しています。");
    ...
    if (!dynamicUse || !indicesUse || !outputUse)
        return base::Failure<CompiledLeaf, std::string>(
            "Level 5実験2 State ResourceUseが検証または実行の契約に違反しています。");
    ...
    interfaceDescription.parameters = {
        ...
        {{2}, "StateOutput", sem::ProgramParameterKind::UnorderedBuffer,
            sem::ShaderStage::Compute, 0, 0, 1}};
    auto program = builder.AddComputeProgram(
        "L5V2.State.Program", std::move(interfaceDescription), {R"hlsl(
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    auto work = builder.AddComputeWorkGeneric(
        "L5V2.State.Work", program.value(), operands, universe, 1, 1);
    if (!work)
    ...
    std::uint32_t height)
{
    ...
    auto dynamicColor = builder.AddDynamicBuffer(
        "L5V2.Texture.Dynamic", totalBytes, MemberBytes, MemberBytes);
    auto output = builder.AddExternalTexture2D(
        "L5V2.Texture.Output", width, height,
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    auto program = builder.AddComputeProgram(
        "L5V2.Texture.Program", std::move(interfaceDescription),
        {source, {}, {}, "CSMain"});
    if (!program)
        return base::Failure<CompiledLeaf, std::string>(program.error());
    const std::array operands = {
    ...
    auto work = builder.AddComputeWorkGeneric(
        "L5V2.Texture.Work", program.value(), operands, universe, 1, 1);
    if (!work)
        return base::Failure<CompiledLeaf, std::string>(work.error());
    ...
    std::uint32_t universe)
{
    ...
    sem::SemanticBuilder builder;
    auto state = builder.AddExternalBuffer("L5V2.Temporal.State", stateBytes, 16);
    auto current = builder.AddExternalBuffer("L5V2.Temporal.Current", 16, 16);
    if (!state || !current)
        return base::Failure<CompiledLeaf, std::string>(
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    auto program = builder.AddComputeProgram(
        "L5V2.Temporal.Program", std::move(interfaceDescription),
        {source, {}, {}, "CSMain"});
    if (!program)
        return base::Failure<CompiledLeaf, std::string>(program.error());
    const std::array operands = {
    ...
    auto work = builder.AddComputeWorkGeneric(
        "L5V2.Temporal.Work", program.value(), operands, 1, 1, 1);
    if (!work)
        return base::Failure<CompiledLeaf, std::string>(work.error());
    return CompileLeaf(std::move(builder).Build(), ComputeProfile(4));
}
    ...
    std::uint32_t universe)
{
    const auto stateBytes = static_cast<std::uint64_t>(universe) * 16u;
    sem::SemanticBuilder builder;
    auto previous = builder.AddExternalBuffer("L5V2.Observation.Previous", 16, 16);
    auto state = builder.AddExternalBuffer("L5V2.Observation.State", stateBytes, 16);
    auto output = builder.AddExternalBuffer("L5V2.Observation.Output", 16, 16);
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    auto program = builder.AddComputeProgram(
        "L5V2.Observation.Program", std::move(interfaceDescription),
        {source, {}, {}, "CSMain"});
    if (!program)
        return base::Failure<CompiledLeaf, std::string>(program.error());
    const std::array operands = {
        ...
        sem::WorkOperand{sem::WorkOperandKind::ProgramParameter, outputUse.value(), {2}}};
    auto work = builder.AddComputeWorkGeneric(
        "L5V2.Observation.Work", program.value(), operands, 1, 1, 1);
    if (!work)
        return base::Failure<CompiledLeaf, std::string>(work.error());
    return CompileLeaf(std::move(builder).Build(), ComputeProfile(6));
}
    ...
    std::uint32_t height)
{
    ...
    if (kind == CandidateKind::VerifiedSparseWorklist)
        indirect = comp::MakeVerifiedCompactWorklistDispatchContractV1(
            stateLeaf, 0, universe, 1);
...
inline dyn::InvocationInputV1 MakeArbitraryInvocationInput(
    ...
    bool modifiedAll)
{
    ...
    input.mode = mode;
    input.activeMembers.assign(activeMembers.begin(), activeMembers.end());
    input.updatePayloads.reserve(activeMembers.size());
    if (modifiedAll)
        input.modifiedSurvivors.assign(activeMembers.begin(), activeMembers.end());
    for (const auto member : activeMembers)
        input.updatePayloads.push_back(
            {member, CanonicalMemberPayload(member, timelineOrdinal)});
    return input;
}

inline std::vector<std::byte> IdentityIndexListBytes(std::uint32_t universe)
{
    std::vector<std::byte> bytes(
        static_cast<std::size_t>(universe) * sizeof(std::uint32_t));
    for (std::uint32_t member = 0; member < universe; ++member)
        std::memcpy(
            bytes.data() + static_cast<std::size_t>(member) * sizeof(member),
