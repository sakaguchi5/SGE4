...
base::Result<ObservationReportV1,std::string> ObserveHierarchyV1(std::span<const corpus::ObservedPointV1> r,std::span<const corpus::ObservedPointV1> a,std::span<const corpus::ObservedPointV1> b,std::span<const corpus::ObservedPointV1> c)
{
    ...
    for(std::size_t i=0;i<r.size();++i){if(!finite(r[i])||!finite(a[i])||!finite(b[i])||!finite(c[i])){++out.nonFiniteCount;++out.mismatchCount;continue;}const double ea=err(a[i],r[i]),eb=err(b[i],r[i]),ec=err(c[i],r[i]);out.maxAbsoluteError=std::max({out.maxAbsoluteError,ea,eb,ec});const double pairwise=std::max({err(a[i],b[i]),err(a[i],c[i]),err(b[i],c[i])});out.maxPairwiseError=std::max(out.maxPairwiseError,pairwise);const double scale=std::max({1.0,std::abs(static_cast<double>(r[i].x)),std::abs(static_cast<double>(r[i].y)),std::abs(static_cast<double>(r[i].z))});if(std::max({ea,eb,ec})>contracts::ObservationAbsoluteToleranceV1+contracts::ObservationRelativeToleranceV1*scale||pairwise>contracts::ObservationPairwiseAbsoluteToleranceV1+contracts::ObservationPairwiseRelativeToleranceV1*scale)++out.mismatchCount;}
    ...
    if(out.maxAxisLengthError>contracts::ObservationRigidityToleranceV1||out.maxOrthogonalityError>contracts::ObservationRigidityToleranceV1||!out.orientationPreserved)++out.mismatchCount;return base::Result<ObservationReportV1,std::string>::Success(out);
}
