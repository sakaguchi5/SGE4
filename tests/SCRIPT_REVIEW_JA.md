...
6. READMEが存在しない `run_sge4_5_final.bat` を案内していました。
   - 正式な `run_sge4_5_freeze.bat` へ修正しました。
...
最初は `run_sge4_5_dev.bat` を実行し、次に `run_sge4_5_gate.bat` を実行してください。

## Qualificationログ保存先の改訂

- `tests/Invoke-SGE4_5Tests.ps1`のログ出力先を`build\test-logs`から`docs\test-logs`へ変更した。
