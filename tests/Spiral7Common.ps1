...
function Invoke-Spiral7Checks([string[]]$Extra = @()) {
    Invoke-PowerShellVerifier 'tools\Verify-SGE4_5Identity.ps1'
    Invoke-PowerShellVerifier 'tools\Verify-Spiral7CU1.ps1' @('-Mode','Regression')
...
function Build-Spiral7Project([string]$Project, [string[]]$Configurations = @('Debug','Release')) {
    $msbuild = Get-MSBuild
