...
if (-not (Test-Path -LiteralPath $solutionPath)) { throw 'Main solution is missing: SemanticGpuEngine4-5.sln' }
...
foreach ($relative in $canonicalRunners) {
    ...
    if ($text -notmatch "SemanticGpuEngine4-5\.sln") { throw "Canonical runner does not use the main solution: $relative" }
    if ($text -match 'SemanticGpuEngine4-5_Level4v1_') { throw "Canonical runner still uses a stage-specific solution: $relative" }
}
...
$obsoleteFiles = @(
    ...
    'SemanticGpuEngine4-5_Level4v1_R1.sln',
    'SemanticGpuEngine4-5_Level4v1_R2.sln',
    'SemanticGpuEngine4-5_Level4v1_R3_R5.sln'
)
