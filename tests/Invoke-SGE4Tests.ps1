...
function Build-Solution {
    ...
    $msbuild = Get-MSBuild
    Invoke-Native $msbuild @(
        (Join-Path $root 'SemanticGpuEngine4.sln'),
        '/m', '/nologo', '/t:Build',
        "/p:Configuration=$Configuration",
        '/p:Platform=x64'
    )
...
function Generate-QualificationPackage {
    $package = Join-Path $testOutputRoot 'SGE4_TieredTests_CommonExperiment.sgep'
...
Set-Content -LiteralPath $logPath -Value "SGE4 tiered test run`nSuite=$Suite`nConfiguration=$Configuration`nStarted=$(Get-Date -Format o)" -Encoding UTF8
...
try {
    ...
    $message = "SGE4 test suite passed: $Suite ($Configuration)"
    Write-Section $message
    Write-Host "Log: $logPath"
    exit 0
}
catch {
    $message = "SGE4 test suite failed: $Suite ($Configuration)`n$($_.Exception.Message)"
