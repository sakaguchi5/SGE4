...
Build-Spiral4Project '143_Spiral4ArchitectureQualificationTests\143_Spiral4ArchitectureQualificationTests.vcxproj'

$output = Join-Path $root 'build\tests\spiral4-cu5'
Remove-Item -Recurse -Force -ErrorAction SilentlyContinue $output
New-Item -ItemType Directory -Force $output | Out-Null

$debugExe = Join-Path $root 'build\bin\x64\Debug\143_Spiral4ArchitectureQualificationTests.exe'
$releaseExe = Join-Path $root 'build\bin\x64\Release\143_Spiral4ArchitectureQualificationTests.exe'
...
Write-Host '============================================================'
Write-Host 'SGE4-5 SPIRAL 4 ARCHITECTURE COMPLETE'
Write-Host 'Completion Unit 5: PASSED'
Write-Host 'WARP corpus: 7 Candidates x 19 Active Counts = 133 cases'
Write-Host 'Inactive-tail sentinel proofs: 133'
Write-Host 'GPU Batch-record proofs: 95'
Write-Host 'Determinism: fresh Debug A / Debug B / Release byte-identical'
Write-Host 'Controlled Recovery: epoch advance, stale rejection, explicit rebind, identical observation'
Write-Host 'Actual Recovery: ID3D12Device5::RemoveDevice -> AwaitingAdapter quarantine'
Write-Host 'Fresh process: same Frozen Candidate family rematerialized and observed'
