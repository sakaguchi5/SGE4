...
$project = [pscustomobject]@{
    Name='172_Spiral6EquivalentSetAuditTests'
...
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) { throw "Equivalent Set Audit project missing: $($project.Path)" }
...
if ($present -ne 0 -and $present -ne 4) { throw 'Partial Solution configuration exists for Equivalent Set Audit.' }
if ($present -eq 0) {
    $block = ''
    foreach ($entry in $required) { $block += "`t`t$entry$newLine" }
    $text = $text.Insert($insert,$block)
}
if (([regex]::Matches($text,[regex]::Escape($project.Path))).Count -ne 1) { throw 'Equivalent Set Audit Solution registration mismatch.' }
$utf8WithBom = New-Object System.Text.UTF8Encoding($true)
[IO.File]::WriteAllText($solutionPath,$text,$utf8WithBom)
Write-Host 'Spiral 6 Equivalent Set Audit project registered in SemanticGpuEngine4-5.sln.'
