...
Require ($m.solutionProjectMutation -eq 'None') 'R0 must not add a solution Project.'
...
foreach($role in @('Spiral1ProofLedger','Spiral2ProofLedger','Spiral3ProofLedger','Spiral4ProofLedger','Spiral5ProofLedger','Spiral6ProofLedger','Spiral7ProofLedger','Spiral7Closure','AcceptedArchitectureQualification','AcceptedObservationalMeasurement')){
    Require ($sourceRoles.ContainsKey($role)) "Required R0 source role missing: $role"
}
...
$ids = @{}
$names = @{}
$allowedOwners = @($m.allowedOwners)
$allowedDestinations = @($m.allowedDestinations)
...
foreach($token in @('NewRuntimeCapability','CanonicalAbiMutation','Schema17Mutation','Runtime17Mutation','BackendSemanticPolicy','GamePolicyInference','HardwareWinnerPolicy','ReferenceProjectDeletion')){
    Require (@($m.forbiddenDuringR0) -contains $token) "R0 forbidden action missing: $token"
}

$ownership = Get-Content -Raw -LiteralPath (Join-Path $root 'docs\level4v2\R0_CANONICAL_LAYER_OWNERSHIP_V1.md') -Encoding UTF8
foreach($token in @('Canonical Semantic','Canonical Invocation','Candidate Proposal','Independent Verifier','Frozen Artifact','Canonical Composition','Canonical Runtime','Canonical Recovery','Single-fact rule')){
    Require ($ownership -match [regex]::Escape($token)) "R0 ownership token missing: $token"
}

$r1 = Get-Content -Raw -LiteralPath (Join-Path $root 'docs\level4v2\R1_CANONICAL_VOCABULARY_ENTRY_CONTRACT_V1.md') -Encoding UTF8
foreach($token in @('R1 implementation = NOT STARTED','ActiveCount','TransitionCount','ReuseCount','UpdateInterval','BatchSize','AffectedBlockCount','TimelineOrdinal','DeviceEpoch','Count-type substitution must fail to compile','Canonical Semantic headers must not depend on D3D12 headers')){
    Require ($r1 -match [regex]::Escape($token)) "R1 entry-contract token missing: $token"
}
...
Write-Host 'Level 4 v2 R0 source identities, invariant ownership and migration destinations passed.'
Write-Host 'Carried invariants: 40. Frozen sources: 16.'
Write-Host 'Implementation and solution mutation: None.'
