...
$projects = @(
    [pscustomobject]@{Name='169_Spiral6ArchitectureQualificationTests';Path='169_Spiral6ArchitectureQualificationTests\169_Spiral6ArchitectureQualificationTests.vcxproj';Guid='{000000A9-0000-5000-8000-0000000000A9}'}
)
foreach ($project in $projects) {
    $projectPath = Join-Path $root $project.Path
    if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) { throw "Spiral 6 CU5 project missing: $($project.Path)" }
...
Write-Host 'Spiral 6 CU5 project registered in SemanticGpuEngine4-5.sln.'
