...
$hierarchy=Get-Content -Raw -LiteralPath (Join-Path $root '80_HierarchySemantic\HierarchySemantic.cpp')
foreach($constant in @('ObservationAbsoluteToleranceV2','ObservationRelativeToleranceV2','ObservationPairwiseAbsoluteToleranceV2','ObservationPairwiseRelativeToleranceV2','ObservationRigidityToleranceV2')){if($observer-notmatch$constant){throw "Observer does not consume versioned tolerance: $constant"}}
if($hierarchy-notmatch[regex]::Escape('ObservationContractIdentityV2')){throw 'Current Spiral 2 semantic is not bound to Observation Contract V2.'}
if($hierarchy-notmatch[regex]::Escape('ObservationContractIdentityV1')){throw 'Historical Observation Contract V1 identity was removed.'}
