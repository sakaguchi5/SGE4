...
Require ($m.completionUnit -eq 'R2' -and $m.status -eq 'CompletePackageSuppliedOwnerGatePending') 'R2 supplied status mismatch.'
...
Require ($m.nextStageOnSuccessfulGate -eq 'R3CanonicalComposition') 'R2 next stage mismatch.'
...
foreach($projectText in @($plannerProject,$verifierProject,$frozenProject)){foreach($forbidden in @('CompositionRuntime','D3D12Backend','Spiral7','SparseTemporal')){Require ($projectText -notmatch [regex]::Escape($forbidden)) "R2 Project has forbidden dependency: $forbidden"}}
...
foreach($token in @('SemanticDefinitionIdentityMismatch','InvocationDefinitionIdentityMismatch','CandidateTargetBindingMismatch','CandidateResourceBindingMismatch','CandidateWriteSetBindingMismatch','OperationCountMismatch','OperationOrdinalMismatch','OperationKindMismatch','OperationSubjectMismatch','PlannerProposalIdentityMismatch')){Require ($verifier -match [regex]::Escape($token)) "R2 rejection boundary missing: $token"}
...
$publicSource=$model+"`n"+(Get-Content -Raw -LiteralPath (Join-Path $root '192_Level4V2CandidatePlanner\CandidatePlanner.h') -Encoding UTF8)+"`n"+(Get-Content -Raw -LiteralPath (Join-Path $root '193_Level4V2IndependentVerifier\IndependentVerifier.h') -Encoding UTF8)+"`n"+$frozen
foreach($forbidden in @('D3D12','DXGI','WARP','RTX4070','GamePolicy','RuntimeCandidatePolicy')){Require ($publicSource -notmatch [regex]::Escape($forbidden)) "Forbidden public R2 token: $forbidden"}

$test=Get-Content -Raw -LiteralPath (Join-Path $root '195_Level4V2AuthorityTests\main.cpp') -Encoding UTF8
foreach($token in @('!std::is_default_constructible_v<canonical::OpaqueVerifiedPlanV1>','MutateIdentity','ExpectError','observed.reserve(18u)','FrozenAuthorityBuilderV1::Freeze','Verifier dependency on Planner project: None')){Require ($test -match [regex]::Escape($token)) "R2 test token missing: $token"}

Write-Host 'Level 4 v2 R2 unified authority, dependency, mutation and replay boundaries passed.'
