...
if($Mode -eq 'Applied'){Require ($m.status -eq 'CompletePackageSuppliedOwnerGatePending') 'R5 supplied status mismatch.'}else{Require ($m.status -eq 'Passed') 'R5 accepted status mismatch.';Require ($m.acceptedCommit -eq '527cc0b2ed4c01c76ebfdef5c277f6c39b654024') 'R5 accepted commit mismatch.'}
...
if($Mode -eq 'Applied'){Require ($input.nextStage -eq 'R6MigrationCorpus') 'R5 handoff does not advance to R6.'}else{Require ($input.r5RuntimeAndRecovery.status -eq 'Passed' -and $input.r5RuntimeAndRecovery.acceptedCommit -eq '527cc0b2ed4c01c76ebfdef5c277f6c39b654024') 'R5 regression acceptance mismatch.';$postR5=@('R6MigrationCorpus','R7ReferenceRetirement','Complete');Require ($postR5 -contains [string]$input.nextStage) 'R5 regression does not see R6-or-later handoff.'}
