...
function Invoke-PowerShellVerifier([string]$RelativeScript, [string[]]$Arguments = @()) {
    Invoke-Checked powershell.exe @(
        ...
        'Bypass',
        '-File',
        (Join-Path $testsRoot $RelativeScript)
    ) + $Arguments
}

function Invoke-Spiral4Checks([string[]]$Extra = @()) {
    Invoke-PowerShellVerifier 'tools\Verify-SGE4_5Identity.ps1'
    Invoke-PowerShellVerifier 'tools\Verify-Spiral4CU1.ps1' @('-Mode', 'Regression')

    foreach ($script in $Extra) {
        Invoke-PowerShellVerifier $script
    }
    ...
    Invoke-Checked powershell.exe @(
        ...
        'Bypass',
        '-File',
        (Join-Path $root 'verify_dependencies.ps1')
    )
}

function Build-Spiral4Project([string]$Project, [string[]]$Configurations = @('Debug','Release')) {
    $msbuild = Get-MSBuild
