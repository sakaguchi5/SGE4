...
[[nodiscard]] std::vector<std::byte> Rewrite(std::vector<SectionInput> sections)
{
    auto result = WriteSectionedArtifact(
        ...
        std::move(sections));
    Require(static_cast<bool>(result), "ABI 2.3破損Corpusの再符号化に失敗しました。");
...
void VerifyAbi2CorruptionRejection(std::span<const std::byte> validBytes)
{
    auto decoded = ReadSectionedArtifact(validBytes,
        artifact::FrozenCompositionAbi2Magic,
        artifact::FrozenCompositionAbi2FormatMajor);
    Require(static_cast<bool>(decoded), "ABI 2.3破損Corpusの正本を読めません。");
    ...
    // 必須Section欠落。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが必須Section欠落を受理しました。");
    }
    ...
    // 未知のRequired Section追加。
    {
        ...
        sections.push_back({9, 1,
            ...
            8, {std::byte{0}}});
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが未知のRequired Sectionを受理しました。");
    }
    ...
    // Section Schema不一致。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 ReaderがManifest Schema不一致を受理しました。");
    }
    ...
    // Leaf Tableのdense IDを壊し、外側Digestは正しく再計算する。
    {
        ...
        PatchU32(table, 0, 1);
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが非dense Leaf IDを受理しました。");
    }
    ...
    // Leaf Tableの先頭offsetをCanonical位置からずらす。
    {
        ...
        PatchU64(table, 40, 8);
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが非Canonical Leaf offsetを受理しました。");
    }
    ...
    // 二つ目のLeaf直前に存在するzero paddingを破壊する。
    {
        ...
        if (firstOffset + firstSize < secondOffset)
        {
            ...
            RequireRejected(Rewrite(std::move(sections)),
                "ABI 2.3 Readerが非zero Leaf paddingを受理しました。");
        }
    }
    ...
    // 埋め込みLeaf Packageを壊し、外側Section／file digestだけを更新する。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが破損したLeaf Packageを受理しました。");
    }
    ...
    // Contract bytesを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが破損したContractを受理しました。");
    }
    ...
    // Verified Decision bytesを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが破損したVerified Decisionを受理しました。");
    }
    ...
    // Verification Certificate bytesを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが破損したVerification Certificateを受理しました。");
    }
    ...
    // ManifestのComposition Core digestを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが不一致Composition Core digestを受理しました。");
    }
    ...
    // ManifestのFrozen Composition identityを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが不一致Frozen Composition identityを受理しました。");
    }
    ...
    // Authority Ledgerだけを書き換え、Composition Coreは維持する。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが不一致Authority Ledgerを受理しました。");
    }
    ...
    // Dynamic identityを壊す。
    {
        ...
        RequireRejected(Rewrite(std::move(sections)),
            "ABI 2.3 Readerが不一致Dynamic identityを受理しました。");
    }
