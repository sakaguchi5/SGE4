...
function Invoke-Spiral5Checks([string[]]$Extra = @()) {
    Invoke-PowerShellVerifier 'tools\Verify-SGE4_5Identity.ps1'
    Invoke-PowerShellVerifier 'tools\Verify-Spiral5CU1.ps1' @('-Mode','Regression')
...
function Build-Spiral5Project([string]$Project, [string[]]$Configurations = @('Debug','Release')) {
    $msbuild = Get-MSBuild
