...
inline constexpr std::uint16_t FrozenInvocationFormatMinor = 6;
inline constexpr std::uint32_t FrozenInvocationManifestSchemaVersion = 7;
...
inline constexpr std::array FrozenInvocationSectionKinds = {
    ...
    FrozenInvocationSectionKind::CompactWorklist};
...
public:
    ...
    [[nodiscard]] const VerifiedCompactWorklistV1& CompactWorklist() const noexcept
    {
        return compactWorklist_;
    }
...
private:
    ...
    FrozenDynamicInvocationPackage(
        ...
        : bytes_(std::move(bytes)), artifact_(std::move(artifact)),
          ...
          compactWorklist_(std::move(compactWorklist)) {}
    ...
    VerifiedCompactWorklistV1 compactWorklist_;
};
