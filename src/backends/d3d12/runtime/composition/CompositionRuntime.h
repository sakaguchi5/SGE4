...
#include "../device/CompositionDeviceDomain.h"
#include "../resources/CompositionSharedResources.h"
...
struct StaticCompositionSubmission final
{
    ...
    std::vector<::sge4::runtime::FrameSubmission> leaves;
};
...
public:
    ...
    [[nodiscard]] ::sge4::runtime::DeviceRuntimeState State() const noexcept { return domain_ ? domain_->State() : ::sge4::runtime::DeviceRuntimeState::Lost; }
    ...
    [[nodiscard]] const artifact::VerifiedFrozenComposition& Artifact() const noexcept { return domain_->Artifact(); }
    [[nodiscard]] std::shared_ptr<::sge4::runtime::IExternalResource> SharedResource(ResourceFlowId) const;
    [[nodiscard]] std::shared_ptr<::sge4::runtime::ICompletionToken> AvailableAfter(ResourceFlowId) const;
...
private:
    ...
    std::vector<std::shared_ptr<::sge4::runtime::ICompletionToken>> endpointTokens_;
    std::vector<SharedResourceInitialData> recoveryInitialResources_;
};

[[nodiscard]] base::Expected<LoadedStaticComposition, StaticRuntimeError> LoadStaticComposition(
    std::span<const std::byte>, d3d12::Executor&, StaticCompositionLoadInput input = {});
...
[[nodiscard]] base::Expected<void, StaticRuntimeError> ValidateStaticCompositionHandleEpoch(
    ...
    const std::shared_ptr<::sge4::runtime::IExternalResource>&,
    const std::shared_ptr<::sge4::runtime::ICompletionToken>&);
}
