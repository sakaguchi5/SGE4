...
[[nodiscard]] base::Expected<std::vector<std::byte>, PackageError> BuildFrozenPackage(
    const D3D12PackageDescription& description);

class D3D12PackageView final
{
public:
    [[nodiscard]] static base::Expected<D3D12PackageView, PackageError> Decode(
    ...
    [[nodiscard]] base::Expected<std::span<const std::byte>, PackageError> ResolveBlob(const BlobRef& blob) const;
...
[[nodiscard]] base::Expected<CreateResourcePayload, PackageError> DecodeCreateResource(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<UploadBufferPayload, PackageError> DecodeUploadBuffer(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<UploadTexturePayload, PackageError> DecodeUploadTexture(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<CreateRootSignaturePayload, PackageError> DecodeCreateRootSignature(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<CreateGraphicsPipelinePayload, PackageError> DecodeCreateGraphicsPipeline(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<CreateComputePipelinePayload, PackageError> DecodeCreateComputePipeline(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<InitializeStatePayload, PackageError> DecodeInitializeState(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<VerifyBufferContentsPayload, PackageError> DecodeVerifyBufferContents(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<VerifyTextureContentsPayload, PackageError> DecodeVerifyTextureContents(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<AcquireSurfaceImagePayload, PackageError> DecodeAcquireSurfaceImage(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<AcquireExternalPayload, PackageError> DecodeAcquireExternal(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<WaitExternalPayload, PackageError> DecodeWaitExternal(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<ApplyDynamicDataPayload, PackageError> DecodeApplyDynamicData(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<TransitionPayload, PackageError> DecodeTransition(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<ExecuteRasterPayload, PackageError> DecodeExecuteRaster(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<ExecuteComputePayload, PackageError> DecodeExecuteCompute(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<CopyBufferPayload, PackageError> DecodeCopyBuffer(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<SignalQueuePayload, PackageError> DecodeSignalQueue(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<WaitQueuePayload, PackageError> DecodeWaitQueue(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<WaitTemporalPayload, PackageError> DecodeWaitTemporal(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<ActivateAliasPayload, PackageError> DecodeActivateAlias(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<ReleaseExternalPayload, PackageError> DecodeReleaseExternal(std::span<const std::byte> payload);
[[nodiscard]] base::Expected<PresentSurfacePayload, PackageError> DecodePresentSurface(std::span<const std::byte> payload);
}
