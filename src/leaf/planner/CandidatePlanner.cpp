...
std::vector<std::byte> EncodeManifest(const CandidateManifest& manifest)
{
    ...
    for (const auto& candidate : manifest.candidates)
    {
        text << "candidate=" << base::ToHex(candidate.plan.identity)
             << "|verified=" << (candidate.verifierAccepted ? 1 : 0)
             << "|diagnostics=" << candidate.verifierViolationCount
...
std::vector<std::size_t> ParetoFrontier(std::span<const CandidateRecord> candidates)
{
    ...
    {
        if (!candidates[candidate].verifierAccepted) continue;
        bool dominated = false;
        for (std::size_t other = 0; other < candidates.size() && !dominated; ++other)
            if (other != candidate && candidates[other].verifierAccepted &&
...
base::Expected<PlanningCandidateSet, PlanningError> PlanCandidates(
    ...
    const planning::CompilerPolicy& policy)
{
    ...
    if (!validated)
        return base::Failure<PlanningCandidateSet, PlanningError>(validated.error());

    auto obligationResult = planning::BuildSemanticObligation(graph, validated.value().analyzed);
    if (!obligationResult)
        return base::Failure<PlanningCandidateSet, PlanningError>(
    ...
    if (policy.budget.maxProposedCandidates == 0 || policy.budget.maxVerifiedCandidates == 0 ||
        ...
        return base::Failure<PlanningCandidateSet, PlanningError>(
            {"candidate-budget", "PlanがCanonicalな順序または識別子規則に違反しています。"});

    auto plans = GenerateCandidatePlans(obligation, contract, policy);
    if (plans.empty())
        return base::Failure<PlanningCandidateSet, PlanningError>(
            {"candidate-generation", "検証または実行の契約に違反しています。"});
    ...
    return base::Success<PlanningCandidateSet, PlanningError>(std::move(output));
}

base::Expected<PlanningSelection, PlanningError> SelectCandidate(
    ...
    const planning::ProfileSelectionContext* profileContext)
{
    ...
    for (std::size_t index = 0; index < manifest.candidates.size(); ++index)
        if (manifest.candidates[index].verifierAccepted &&
            !manifest.candidates[index].packageExecutionDigestHex.empty())
            selectable.push_back(index);

    if (selectable.empty())
        return base::Failure<PlanningSelection, PlanningError>(
            {"plan-verification", "検証または実行の契約に違反しています。"});
    ...
    if (profile != nullptr)
    {
        if (profileContext == nullptr || profile->version != 1 ||
            ...
            return base::Failure<PlanningSelection, PlanningError>(
                {"profile-validation", "Contractが検証または実行の契約に違反しています。"});
        ...
        if (found == selectable.end())
            return base::Failure<PlanningSelection, PlanningError>(
                {"profile-validation", "検証または実行の契約に違反しています。"});
        ...
        if (base::ToHex(profile->packageExecutionDigest) != candidate.packageExecutionDigestHex)
            return base::Failure<PlanningSelection, PlanningError>(
                {"profile-validation", "Packageが検証または実行の契約に違反しています。"});
        const auto selectedIndex = *found;
        selectable.clear();
        selectable.push_back(selectedIndex);
    }
    ...
    output.manifest = std::move(manifest);
    return base::Success<PlanningSelection, PlanningError>(std::move(output));
}
}
