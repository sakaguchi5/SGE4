               ...
               IdFn id, const char* label, std::vector<Diagnostic>& diagnostics)
{
    ...
    for (std::size_t index = 0; index < values.size(); ++index)
    {
        ...
        if (!valueId.IsValid())
        {
            Push(diagnostics, std::string(label) + "入力または内部状態が無効であるか、契約条件を満たしていません。", static_cast<std::uint32_t>(index));
            valid = false;
            continue;
        }
                      ...
                      std::vector<Diagnostic>& diagnostics)
{
    ...
    Push(diagnostics, "Resourceが検証または実行の契約に違反しています。", source);
}
                     ...
                     std::vector<Diagnostic>& diagnostics)
{
    ...
    if (program.kind == ProgramKind::Raster)
    {
        ...
        for (const auto& input : program.interface.vertexInputs)
        {
            if (input.componentCount == 0 || input.componentCount > 4 ||
                ...
                Push(diagnostics, "入力または内部状態に重複または二重処理があります。", source);
            ...
            for (const auto& [otherBegin, otherEnd] : ranges)
                if (input.byteOffset < otherEnd && otherBegin < end)
                    Push(diagnostics, "入力または内部状態が検証または実行の契約に違反しています。", source);
        ...
        if (requiredStride > program.interface.vertexStrideBytes ||
            ...
            Push(diagnostics, "入力または内部状態の値または参照範囲が許容範囲外です。", source);
    }
    ...
    for (const auto& parameter : program.interface.parameters)
    {
        if (!parameter.id.IsValid() || parameter.id.value >= program.interface.parameters.size() ||
            ...
            Push(diagnostics, "ProgramがCanonicalな順序または識別子規則に違反しています。", source);
        ...
        case ProgramParameterKind::SampledTexture:
            ...
            if (parameter.stage != ShaderStage::Pixel || parameter.requiredBytes != 0 ||
                ...
                Push(diagnostics, "Textureが検証または実行の契約に違反しています。", source);
            break;
        ...
        if (registerClass != 0 &&
            ...
            Push(diagnostics, "Shaderが検証または実行の契約に違反しています。", source);
    }
}
                         ...
                         std::uint32_t source, std::vector<Diagnostic>& diagnostics)
{
    ...
    case ViewRole::CopyDestination:
        ...
        break;
    default:
        Push(diagnostics, "ResourceUseが検証または実行の契約に違反しています。", source);
        break;
    }
    ...
    for (const auto& operand : work.operands)
    {
        ...
        case WorkOperandKind::ProgramParameter:
        {
            ...
            if (!ParameterRoleMatches(parameter->kind, use->role))
                Push(diagnostics, "ResourceUseがCanonicalな順序または識別子規則に違反しています。", source);
            else
            {
                ...
                if (resourceFound != resources.end() && parameter->kind == ProgramParameterKind::ConstantBuffer)
                {
                    ...
                    if (resource->dynamicData.requiredBytes != parameter->requiredBytes ||
                        ...
                        Push(diagnostics, "ResourceがCanonicalな順序または識別子規則に違反しています。", source);
                }
            }
            break;
        }
        case WorkOperandKind::VertexData:
            ...
            if (use->role != ViewRole::VertexData) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        case WorkOperandKind::ColorAttachment:
            ++color; colorUse = use;
            if (use->role != ViewRole::ColorAttachment) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        case WorkOperandKind::DepthAttachment:
            ++depth;
            if (use->role != ViewRole::DepthAttachment) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        case WorkOperandKind::PresentSource:
            ++present; presentUse = use;
            if (use->role != ViewRole::PresentSource) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        case WorkOperandKind::CopySource:
            ++copySource; copySourceUse = use;
            if (use->role != ViewRole::CopySource) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        case WorkOperandKind::CopyDestination:
            ++copyDestination; copyDestinationUse = use;
            if (use->role != ViewRole::CopyDestination) Push(diagnostics, "入力または内部状態がCanonicalな順序または識別子規則に違反しています。", source);
            break;
        default:
...
base::Expected<AnalyzedGraph, std::vector<Diagnostic>> Analyze(const SemanticGraph& graph)
{
    std::vector<Diagnostic> diagnostics;
    if (graph.resources.empty()) Push(diagnostics, "Resourceが検証または実行の契約に違反しています。");
    if (graph.works.empty()) Push(diagnostics, "Workが検証または実行の契約に違反しています。");
    ...
    for (std::size_t index = 0; index < graph.resourceUses.size(); ++index)
    {
        ...
        if (!use.resource.IsValid() || found == resources.end())
            Push(diagnostics, "ResourceUseが検証または実行の契約に違反しています。", static_cast<std::uint32_t>(index));
        else
    ...
    for (std::size_t index = 0; index < graph.works.size(); ++index)
    {
        ...
        if (programId.IsValid())
        {
            ...
            else Push(diagnostics, "Programが検証または実行の契約に違反しています。", static_cast<std::uint32_t>(index));
        }
        ...
        for (const auto dependency : work.dependencies)
            if (!dependency.IsValid() || dependency == work.id || works.find(dependency.value) == works.end())
                Push(diagnostics, "Workが検証または実行の契約に違反しています。", static_cast<std::uint32_t>(index));
    }
    ...
    for (const auto& [id, use] : uses)
    {
        ...
        if (ownerCount != 1)
            Push(diagnostics, "ResourceUseが検証または実行の契約に違反しています。", id);
        ...
        if (resource != resources.end() && resource->second->lifetime == LifetimeIntent::Preparation)
            Push(diagnostics, "Resourceが検証または実行の契約に違反しています。", id);
    }

    for (const auto& [id, program] : programs)
        if (programOwners[id] == 0)
            Push(diagnostics, "Programが検証または実行の契約に違反しています。", id);
    ...
    for (const auto& resource : graph.resources)
    {
        ...
        {
            Push(diagnostics, "Resourceが検証または実行の契約に違反しています。", resource.id.value);
            continue;
        }
        ...
        if (resource.lifetime == LifetimeIntent::Preparation ||
            ...
            Push(diagnostics, "Packageが検証または実行の契約に違反しています。", resource.id.value);
        const auto* candidate = preparation->second;
        if (candidate->lifetime != LifetimeIntent::Preparation || candidate->kind != resource.kind)
            Push(diagnostics, "入力または内部状態が検証または実行の契約に違反しています。", resource.id.value);
            ...
            Push(diagnostics, "Bufferが検証または実行の契約に違反しています。", resource.id.value);
            ...
            Push(diagnostics, "Texture2Dが検証または実行の契約に違反しています。", resource.id.value);
    }
    for (const auto& resource : graph.resources)
    ...
    if (!diagnostics.empty())
        return base::Failure<AnalyzedGraph, std::vector<Diagnostic>>(std::move(diagnostics));
    ...
    while (canonicalWorkOrder.size() != works.size())
    {
        ...
        if (selected == nullptr)
        {
            Push(diagnostics, "入力または内部状態が検証または実行の契約に違反しています。");
            break;
        }
    ...
    return base::Success<AnalyzedGraph, std::vector<Diagnostic>>(std::move(output));
}
}
