...
base::Expected<std::vector<std::byte>, FrozenCompositionError> FrozenCompositionWriter::Write(
    FrozenCompositionBuildInput input)
{
    try
    {
        ...
        {
            return base::Failure<std::vector<std::byte>, FrozenCompositionError>(Error(
        ...
        for (std::size_t index = 0; index < input.leaves.size(); ++index)
        {
            ...
            if (!package)
            {
                return base::Failure<std::vector<std::byte>, FrozenCompositionError>(Error(
                    ...
                    "検証または実行の契約に違反しています。" + package.error().message,
        ...
        return base::Success<std::vector<std::byte>, FrozenCompositionError>(std::move(bytes));
    }
