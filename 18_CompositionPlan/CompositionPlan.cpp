...
namespace sge4_5::composition::canonical::planning
{
namespace
{
...
base::Result<package::d3d12_v13::ResourceState, PlanError>
ReadState(base::BinaryReader& reader)
{
    ...
    return base::Result<package::d3d12_v13::ResourceState, PlanError>::Success(state);
}
...
base::Result<RawCompositionPlan, PlanError>
ProposeCompositionPlan(const ValidatedCompositionContract& validatedContract)
{
    ...
    if (!scheduleResult)
        return base::Result<RawCompositionPlan, PlanError>::Failure(
    ...
    if (!shape)
        return base::Result<RawCompositionPlan, PlanError>::Failure(shape.Error());
    return base::Result<RawCompositionPlan, PlanError>::Success(std::move(plan));
}
...
DeserializeRawCompositionPlan(std::span<const std::byte> bytes)
{
    if (bytes.size() < 8 + base::Digest256{}.size() * 2)
        return Failure<RawCompositionPlan>("plan/read", "raw plan bytes are too small");
    ...
    if (!magic || !version || magic.Value() != PlanMagic || version.Value() != PlanVersion)
        return Failure<RawCompositionPlan>("plan/read", "raw plan magic or version is invalid");

    RawCompositionPlan plan;
    auto contractIdentity = ReadDigest(reader);
    if (!contractIdentity) return base::Result<RawCompositionPlan, PlanError>::Failure(contractIdentity.Error());
    ...
    if (!allocationCount || !scheduleCount || !bindingCount || !handoffCount ||
        ...
        return Failure<RawCompositionPlan>("plan/read", "raw plan counts are truncated");
    ...
    if (allocationCount.Value() > MaximumRecords || scheduleCount.Value() > MaximumRecords ||
        ...
        return Failure<RawCompositionPlan>("plan/read", "raw plan count exceeds the supported limit");
    ...
    for (std::uint32_t i = 0; i < allocationCount.Value(); ++i)
    {
        ...
        if (!resource || !ownership || !kind || !format || !size)
            return Failure<RawCompositionPlan>("plan/read", "allocation record is truncated");
        plan.allocations.push_back({
    ...
    for (std::uint32_t i = 0; i < scheduleCount.Value(); ++i)
    {
        ...
        if (!ordinal || !leaf)
            return Failure<RawCompositionPlan>("plan/read", "schedule record is truncated");
    ...
    for (std::uint32_t i = 0; i < bindingCount.Value(); ++i)
    {
        ...
        if (!endpoint || !resource || !leaf || !slot || !access || !reserved || reserved.Value() != 0)
            return Failure<RawCompositionPlan>("plan/read", "binding record is truncated or reserved bits are set");
        plan.bindings.push_back({
    ...
    for (std::uint32_t i = 0; i < handoffCount.Value(); ++i)
    {
        ...
        if (!resource || !producer || !consumer || !producerLeaf || !consumerLeaf || !outgoing || !incoming)
            return Failure<RawCompositionPlan>("plan/read", "handoff record is truncated");
        plan.handoffs.push_back({
    ...
    for (std::uint32_t i = 0; i < signalCount.Value(); ++i)
    {
        ...
        if (!id || !resource || !producer || !leaf || !ordinal)
            return Failure<RawCompositionPlan>("plan/read", "signal record is truncated");
    ...
    for (std::uint32_t i = 0; i < waitCount.Value(); ++i)
    {
        ...
        if (!id || !signal || !resource || !consumer || !leaf || !ordinal)
            return Failure<RawCompositionPlan>("plan/read", "wait record is truncated");
    ...
    if (!recoveryVersion || !recoveryLeafCount || !recoveryResourceCount ||
        ...
        return Failure<RawCompositionPlan>("plan/read", "recovery header is invalid");
    ...
    {
        auto leaf = reader.ReadU32();
        if (!leaf) return Failure<RawCompositionPlan>("plan/read", "recovery Leaf list is truncated");
        plan.recovery.recreateLeaves.push_back({leaf.Value()});
    }
    for (std::uint32_t i = 0; i < recoveryResourceCount.Value(); ++i)
    {
        auto resource = reader.ReadU32();
        if (!resource) return Failure<RawCompositionPlan>("plan/read", "recovery Resource list is truncated");
        plan.recovery.recreateResources.push_back({resource.Value()});
    }

    auto identity = ReadDigest(reader);
    if (!identity) return base::Result<RawCompositionPlan, PlanError>::Failure(identity.Error());
    plan.identity = identity.Value();
    if (reader.Remaining() != 0)
        return Failure<RawCompositionPlan>("plan/read", "raw plan has trailing bytes");

    auto validation = ValidateRawCompositionPlanShape(plan);
    if (!validation)
        return base::Result<RawCompositionPlan, PlanError>::Failure(validation.Error());
    return base::Result<RawCompositionPlan, PlanError>::Success(std::move(plan));
}
}
