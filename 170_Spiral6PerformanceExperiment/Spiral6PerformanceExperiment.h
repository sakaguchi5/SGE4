...
enum class EquivalentSetAuditScheduleV1 : std::uint32_t
{
    BalancedInterleaved = 1,
    FixedPatternMajor = 2
};

enum class EquivalentSetAuditGroupV1 : std::uint32_t
{
    K1PrefixUniform = 1,
    K4096AllPatterns = 2
};

struct EquivalentSetAuditConfigV1 final
{
    EquivalentSetAuditScheduleV1 schedule =
        EquivalentSetAuditScheduleV1::BalancedInterleaved;
    std::uint32_t roundCount = 1;
    std::uint32_t adapterIndex = 0;
};

struct EquivalentSetAuditRunSummaryV1 final
{
    EquivalentSetAuditScheduleV1 schedule =
        EquivalentSetAuditScheduleV1::BalancedInterleaved;
    std::string adapterDescription;
    std::uint32_t sampleCount = 0;
    base::Digest256 k1SparseSetIdentity{};
    base::Digest256 k4096SparseSetIdentity{};
    base::Digest256 csvIdentity{};
    base::Digest256 runReportIdentity{};
};

[[nodiscard]] std::string EquivalentSetAuditScheduleNameV1(
    EquivalentSetAuditScheduleV1 value);

[[nodiscard]] base::Result<void, std::string>
RunEquivalentSetAuditScheduleSelfTestV1();

[[nodiscard]] base::Result<EquivalentSetAuditRunSummaryV1, std::string>
RunEquivalentSetAuditV1(
    const EquivalentSetAuditConfigV1& config,
    const std::filesystem::path& outputDirectory);

enum class TransitionProbeModeV1 : std::uint32_t
{
    CheckpointOnly = 1,
    BracketedCalibration = 2
};

enum class TransitionProbeWindowV1 : std::uint32_t
{
    NotObserved = 0,
    BetweenSamples = 1,
    ValidationOrResourceCreation = 2,
    CommandRecording = 3,
    SubmitOrBeforeMainGpu = 4,
    DuringMainGpu = 5,
    AfterMainGpu = 6,
    UnresolvedInsideSample = 7
};

struct TransitionProbeConfigV1 final
{
    TransitionProbeModeV1 mode = TransitionProbeModeV1::CheckpointOnly;
    std::uint32_t sampleCount = 768;
    std::uint32_t adapterIndex = 0;
    std::uint32_t baselineSampleCount = 32;
    std::uint32_t sustainedSampleCount = 8;
    double transitionRatio = 2.5;

    // NVIDIA-only diagnostic extension. The NVML DLL is loaded dynamically so
    // neither the CUDA Toolkit nor an import library is required to build SGE.
    bool enableNvmlTelemetry = false;
    bool requireNvmlTelemetry = false;
    std::uint32_t nvmlDeviceIndex = 0;
    std::uint32_t nvmlPollingIntervalMilliseconds = 5;

    // Diagnostic-only causal control. This can require Windows Developer Mode
    // and intentionally does not participate in the normal CU6 ranking.
    bool requestStablePowerState = false;
};

struct TransitionProbeRunSummaryV1 final
{
    TransitionProbeModeV1 mode = TransitionProbeModeV1::CheckpointOnly;
    std::string adapterDescription;
    std::uint32_t sampleCount = 0;
    bool transitionObserved = false;
    std::uint64_t firstSlowOrdinal = 0;
    TransitionProbeWindowV1 transitionWindow = TransitionProbeWindowV1::NotObserved;
    double baselineSentinelNanoseconds = 0.0;
    double firstSlowSentinelNanoseconds = 0.0;
    base::Digest256 sparseSetIdentity{};
    base::Digest256 csvIdentity{};
    base::Digest256 runReportIdentity{};

    bool nvmlTelemetryEnabled = false;
    bool nvmlAvailable = false;
    bool stablePowerStateRequested = false;
    bool stablePowerStateApplied = false;
    std::uint32_t nvmlPollingIntervalMilliseconds = 0;
    std::uint64_t nvmlTimelineRecordCount = 0;
    std::string nvmlDeviceName;
    std::string nvmlDeviceUuid;
    std::string nvmlTransitionDiagnosis;
    base::Digest256 nvmlTimelineIdentity{};
};

[[nodiscard]] std::string TransitionProbeModeNameV1(TransitionProbeModeV1 value);
[[nodiscard]] std::string TransitionProbeWindowNameV1(TransitionProbeWindowV1 value);

[[nodiscard]] base::Result<void, std::string>
RunTransitionProbeSelfTestV1();

[[nodiscard]] base::Result<TransitionProbeRunSummaryV1, std::string>
